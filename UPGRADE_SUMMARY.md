...
| **Connectivity** | None | WiFi + Web | Live telemetry |
...
└── src/
    ...
    ├── telemetry/
    │   └── WiFiTelemetry.h/cpp   # UDP + Web server
    ├── alerts/
...
// Web dashboard
http://192.168.4.1/status  // JSON API
```
...
3. **Data Processing:**
   ...
   # New - Binary
   import struct
   packets = read_binary('rally_000.bin')
   ```
...
**Bottom Line:** The RTOS version provides professional-grade telemetry capabilities with real-time guarantees, while maintaining the simplicity of the original Arduino-style programming model.
